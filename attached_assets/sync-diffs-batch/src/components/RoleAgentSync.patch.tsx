...
import { diffRoleAgent, roleToSuggestedAgent, BASE_POLICIES, BASE_TOOLS } from '../lib/diff'
...
export default function RoleAgentSync(){
  ...
  const bulkGenerate = async ()=>{
    if(!unmapped.length){ alert('No missing Agent Specs.'); return }
    setLoading(true); setStatus('Generating specs...')
    try{
      for(const r of unmapped){
        await upsertAgentSpec(roleToSuggestedAgent(r))
      }
      setStatus('Generated '+unmapped.length+' specs.'); await refresh()
    }catch(e:any){ alert('Error generating specs: '+(e?.message||e)) } finally { setLoading(false) }
  }
  ...
  const applyAllForHandle = async (handle:string, diffs:any[])=>{
    ...
    try{
      ...
      await upsertAgentSpec(updated)
      await refresh()
    }catch(e:any){ alert('Apply all failed: '+(e?.message||String(e))) } finally { setLoading(false) }
  }

  const fixAllDiffs = async ()=>{
    const allWithSuggestions = rows.filter(row=>hasSuggestions(row.diffs))
    if(!allWithSuggestions.length){ alert('No suggestions to apply.'); return }
    ...
    try{
      ...
      await refresh()
    }catch(e:any){ alert('Fix all failed: '+(e?.message||String(e))) } finally { setLoading(false) }
  }
  ...
  return (
    <section className="section">
      ...
      <div style={{display:'flex', gap:8, flexWrap:'wrap', alignItems:'center'}}>
        <button className="btn" data-pod="product" onClick={bulkGenerate} disabled={loading}>
