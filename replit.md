...
A Lifestyle Hero Image Generation System (`POST /api/work-items/:id/generate-lifestyle-heroes`) automates creation of hero images from existing Lifestyle Pack v2 data. The system uses OpenAI DALL-E-3 HD to generate master images (1792x1024) based on shot_board specifications, then crops/resizes using Sharp to Desktop (1920x800), Tablet (960x600), and Mobile (1080x1350) dimensions. Images are uploaded to S3 with exact filenames from the pack's export_plan. The endpoint supports dual authentication (session + API token), conditional RBAC for session users (ops_admin, design_pod, admin), and includes dryRun mode for planning and overwrite flag for regeneration. Additionally, the endpoint accepts a `shotIds` array parameter for selective shot regeneration. Ops events are logged for auditing actual generations (dry runs excluded). The UI includes a "Generate Lifestyle Hero Images" button in the Work Item Actions panel with comprehensive error handling and dynamic success toasts showing created/skipped counts and shot IDs. A Lifestyle Hero Preview component displays generated hero images (Desktop 1920×800 crops) directly in the Work Item detail view below the packs panel, showing image thumbnails with shot IDs, SKUs, and dimensions. The preview component has been upgraded to interactive "mini control panels" for each shot, enabling per-shot reference image uploads (via `POST /api/work-items/:id/lifestyle-hero-references`), text instruction inputs for regeneration refinements, and selective regeneration. Text instructions are stored per-shot in the `lifestyle_hero_shot_settings` table and automatically incorporated into DALL-E-3 prompts during regeneration via the `buildPromptWithInstructions()` helper, which appends sanitized user refinements (600-char limit) with guard text to preserve base shot-board specifications. The instruction inputs feature debounced autosave (750ms), optimistic UI updates, inline status indicators ("Saving…" / "Saved Xs ago"), and comprehensive error handling with retry capability. Reference images are stored in S3 at `fcc/lifestyle_refs/{workItemId}/{shotId}/` with metadata tracked in the `lifestyle_hero_references` table. The component displays uploaded reference thumbnails (max 3 visible with overflow count) and provides individual upload and regenerate buttons per shot with comprehensive loading states and toast feedback. API endpoints include `GET /api/work-items/:id/lifestyle-hero-instructions` for fetching all shot instructions and `PATCH /api/work-items/:id/lifestyle-hero-instructions/:shotId` for upserting per-shot instructions. Images are served via the `/img/*` CDN route which proxies S3 assets with proper caching headers.
...
### Database Schema
Core tables include agents, agent_specs, projects, tasks, ideas, decisions, business_units, knowledge_items, work_orders, operations_events, work_item_packs, and brand_products. E-commerce tables include affiliates, aff_clicks, aff_attributions, inventory_products, and inventory_events. Operational configuration tables include ops_settings and ops_settings_audit. File attachment tables include work_item_files, alt_texts, lifestyle_hero_references, and lifestyle_hero_shot_settings.
