...
function writeText(fp: string, content: string) {
...
async function main() {
  ...
  // Single-source release metadata + notes
  ...
  let releaseTagPrefix = `i3/${projectKey}/`;
  ...
  if (explicitNotes) {
    ...
    releaseTag = getLatestI3Tag(projectKey) ?? null; // traceability only
    releaseRange = null;
  ...
  // Write artifacts into staging dir (EXPORTS_DIR root)
  ...
  // Tar everything in exports that is relevant (tar root includes MANIFEST + artifacts)
  const manifestPath = path.join(EXPORTS_DIR, "MANIFEST.json");
  writeText(manifestPath, JSON.stringify(manifest, null, 2) + "\n");

  // Create tar.gz with manifest + artifacts + (optional) repo-specific exports later
  // For now: only core files; you can add more paths to include later.
  const cwd = EXPORTS_DIR;
  execSync(`tar -czf "${outTar}" "MANIFEST.json" "RELEASE_NOTES.md" "MISSION.md"`, { stdio: "inherit", cwd });

  const tarSha = sha256File(outTar);
  console.log(JSON.stringify({ ok: true, bundleName, tarPath: path.relative(process.cwd(), outTar), sha256: tarSha }, null, 2));
}
