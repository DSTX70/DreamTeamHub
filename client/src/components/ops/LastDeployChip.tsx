...
export default function LastDeployChip() {
  ...
  return (
    <a
      ...
      data-testid="chip-last-deploy"
      title={`Deployed ${label} by ${d.actor || "ci"} at ${new Date(d.ts).toLocaleString()}`}
    >
      <span className="font-medium">Last Deploy</span>
      <span className="text-gray-600" data-testid="text-deploy-label">{label}</span>
      <span className="text-gray-400">·</span>
      <span className="text-gray-600" data-testid="text-deploy-time">{new Date(d.ts).toLocaleString()}</span>
    </a>
  );
}
