...
router.get("/", async (_req: Request, res: Response) => {
  const { rows } = await db.query(`SELECT id, family, label, tips, augment_lines AS "augmentLines", enabled, updated_at AS "updatedAt" FROM llm_presets ORDER BY updated_at DESC`);
...
router.delete("/:id", async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  await db.query(`DELETE FROM llm_presets WHERE id=$1`, [id]);
  res.json({ ok: true });
});
