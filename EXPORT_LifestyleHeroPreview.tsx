/**
 * LifestyleHeroPreview.tsx
 * 
 * A React component for Dream Team Hub that provides:
 * - Lifestyle hero image preview grid for work items
 * - Per-shot image generation controls with DALL-E-3 integration
 * - Reference image uploads to S3
 * - Text instruction inputs for AI regeneration refinements
 * - Complete version management system (create, preview, activate versions)
 * - Image Status strip showing Desktop/Tablet/Mobile availability
 * 
 * Key Features:
 * 1. VersionSelector: Expandable list of image versions per shot with thumbnail previews
 * 2. ShotPreviewCard: Individual shot management with regeneration controls
 * 3. Image Status Strip: Validates Desktop/Tablet/Mobile image availability with OK/Missing badges
 * 4. Debounced instruction autosave (750ms) with status indicators
 * 5. Memory-safe async image checking with generation tokens to prevent race conditions
 * 
 * Architecture Notes:
 * - Uses TanStack Query v5 for data fetching with 30s refetch intervals
 * - Generation tokens (checkGenerationRef) prevent stale async updates from rapid rechecks
 * - activeImagesRef tracks Image objects for proper cleanup on unmount
 * - Custom useDebouncedCallback hook for instruction autosave
 * 
 * @author Dream Team Hub / i³ collective
 * @version 2.0.0
 */
 ...
 * Prevents creating new debounced functions on every render while
 * still using the latest callback via ref.
 */
...
/**
 * VersionSelector Sub-component
 * 
 * Displays an expandable list of image versions for a shot.
 * Features:
 * - Lazy-loaded version fetching (only when expanded)
 * - Thumbnail previews for each version
 * - Active version indicator with checkmark
 * - "Set Active" button for non-active versions
 * - Error/retry handling for failed version loads
 */
...
/**
 * Main LifestyleHeroPreview Component
 * 
 * Displays a grid of lifestyle hero image previews for a work item.
 * Fetches pack data, references, and instructions from the backend.
 * 
 * State Management:
 * - packs: Work item pack data including export_plan
 * - references: Reference images uploaded for each shot
 * - instructionsState: Per-shot regeneration instructions with autosave
 * - selectedVersionIds: Currently previewed version per shot
 */
export function LifestyleHeroPreview({ workItemId }: LifestyleHeroPreviewProps) {
  ...
  // Regenerate mutation - calls DALL-E-3 via backend
  ...
  // Save instructions mutation with optimistic updates
  ...
  // Debounced save handler - 750ms delay
  ...
  // Group export plan rows by shot_id
  ...
  // Build preview data for each shot
  const previews = Array.from(byShot.entries()).map(([shotId, rows]) => {
    ...
    return {
      ...
      sku: desktop.sku,
      row: desktop,
      rows, // Pass all rows for Desktop/Tablet/Mobile status checking
    };
  });
  ...
  return (
    <div className="mt-6 rounded-xl border border-border bg-card p-4">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3" data-testid="pack-details-lifestyle">
...
/**
 * Image size status type for tracking Desktop/Tablet/Mobile availability
 */
type SizeStatus = {
  sizeLabel: string;
  width: number;
...
/**
 * ShotPreviewCard Component
 * 
 * Individual shot card with:
 * - Main image preview (from active version or latest generation)
 * - Version selector for switching between image versions
 * - Reference image thumbnails
 * - Regeneration instructions textarea with autosave
 * - Image Status strip showing Desktop/Tablet/Mobile availability
 * - Upload ref and Regenerate action buttons
 * 
 * Memory Safety:
 * - Uses checkGenerationRef to track async image checks and discard stale updates
 * - Uses activeImagesRef to track Image objects for cleanup on unmount
 * - Cleans up Image object event handlers on unmount and before new checks
 */
function ShotPreviewCard({
  ...
  // Image status tracking with memory safety
  const [sizeStatuses, setSizeStatuses] = useState<SizeStatus[]>([]);
  const checkGenerationRef = useRef(0); // Prevents stale async updates
  const activeImagesRef = useRef<HTMLImageElement[]>([]); // Tracks Image objects for cleanup
  ...
  /**
   * Check image availability for all sizes with proper cleanup
   * Uses generation tokens to prevent race conditions from rapid rechecks
   */
  const checkImagesForShot = useCallback(() => {
    ...
    // Clean up previous Image objects to prevent memory leaks
    ...
    // Initialize status array for all sizes
    const sizes: SizeStatus[] = rows.map((r) => ({
      ...
      ok: null, // null = still checking
    }));

    setSizeStatuses(sizes);

    // Check each size asynchronously
  ...
  return (
    ...
    >
      ...
      {/* Image Status Strip - Shows Desktop/Tablet/Mobile availability */}
